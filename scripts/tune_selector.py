...
run on the VALID split (never test) so the final test-set numbers reported to
the user aren't themselves the product of tuning on them.

For each (case_id, dataset) group we build the case-level inputs that don't
depend on selector hyperparameters (candidates, graded user/item aspect
weights, allowed set) ONCE and reuse them across the whole grid -- the model
prediction and per-case aggregation are the expensive parts, not re-running
the greedy loop itself.

For each config we compute per-(dataset,k) aggregate sem_f1 / aspect_p /
aspect_r / aspect_f1 / noise / redundancy (same eval_row logic as
evaluate_evidence.py, in-memory, no file I/O), then:
  1. Find the Pareto frontier over (sem_f1 max, aspect_f1 max, noise min,
     redundancy min), averaged across datasets at k in {3,5,user_avg} (k=1 is
     coverage-mechanism-independent, see select_topk.py, so excluded from
     tuning).
  2. Among Pareto-optimal configs, pick the one maximizing F_beta(aspect_p,
     aspect_r; beta=1.5) -- explicitly weighting recall over precision, since
     the shipped config was found to be over-indexed on precision -- subject to
     noise <= --noise-ceiling and sem_f1 >= --sem-f1-floor.
"""
...
def main():
    ...
    # Single fixed values by default = the winning config from the previous
    # tuning round (results/selector_tuning.csv); pass e.g. --gate-percentile-grid
    # 0.0 0.3 to re-sweep a dimension instead of holding it fixed.
    ...
    ap.add_argument('--dataset', default=None,
                     help='Restrict to one dataset (baby/musical/cellphone) instead of the whole split at once -- '
                          'the per-case `cases` list built below is held for the whole grid search, and at full-split '
                          'size (all datasets) was found to no longer fit the server\'s 32GB cgroup limit for the '
                          'larger test split. Run once per dataset and combine (weighted by case count) to verify '
                          'a config on the full split.')
    args = ap.parse_args()
    ...
    # so copying it just doubles peak memory for nothing.
