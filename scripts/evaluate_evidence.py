...
def load_embeddings(path: str | Path, needed_ids: set[str] | None = None):
    p = Path(path)
...
def main():
    ...
    pred_files = list(iter_prediction_files(Path(args.pred_dir)))
    needed_ids: set[str] = set()
    for f in pred_files:
        for p in read_jsonl(f):
            needed_ids.update(p.get('selected_sentence_ids') or [])
            gt_info = case_gt.get(p['case_id'])
            if gt_info:
                needed_ids.update(gt_info['gt_sentence_ids'])
    embs = load_embeddings(args.embeddings, needed_ids)
    rows = []
    for f in pred_files:
        for p in tqdm(read_jsonl(f), desc=f'Eval {f.name}'):
